import test from 'node:test';import assert from 'node:assert/strict';import {confidence,freshnessWeight,rank,rankFuel} from '../app/trust.js';
...
test('fuel ranking prefers verified requested outlet model',()=>{const fuel={...base,products:['petrol'],model:'COCO',verifiedModel:true};const results=rankFuel([{...fuel,id:'a'},{...fuel,id:'b',model:'DODO',verifiedModel:false}],'petrol','COCO');assert.equal(results[0].id,'a')});
